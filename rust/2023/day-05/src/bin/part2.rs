        ...
        .map(|seed_range| {
            seed_range
                ...
                .map(|seed| {
                    ...
                    let fertilizer =
                        match_input_to_map(soil, &soil_to_fertilizer);
                    let water =
                        match_input_to_map(fertilizer, &fertilizer_to_water);
                    let light = match_input_to_map(water, &water_to_light);
                    let temperature =
                        match_input_to_map(light, &light_to_temperature);
                    let humidity = match_input_to_map(
                        temperature,
...
fn parse_seeds(s: &str) -> Vec<Range<isize>> {
    ...
    let seeds_pairs: Vec<_> = raw_seeds
        .split_whitespace()
